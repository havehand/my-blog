<template>
  <div class="project-container" ref="projectContainer" v-loading="loading">
    ...
    <Empty :size="100" v-if="data.length === 0 && !loading"></Empty>
  </div>
</template>
...
export default {
  ...
  methods: {
    handleScroll(){
      this.$bus.$emit("projectScroll", this.$refs.projectContainer);
    },
    handleSetMainScroll(top){
      this.$refs.projectContainer.scrollTop = top;
    }
  },
  beforeDestroy() {
    this.$bus.$emit("projectScroll", undefined);
    this.$refs.projectContainer.removeEventListener("scroll", this.handleScroll);
